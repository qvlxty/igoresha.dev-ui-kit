...
Функция провайдит следующие элементы:

- ContextMenu - компонент 
- $payload - `Store<T>` стор переданного пейлоада 
- openMenu - `Event<T>` ивент открытия контекстного меню
- closeMenu - `Event<void>` ивент форсированного закрытия контекстного меню
    ...
    return (
        <>
            ...
            <ContextMenu items={[
                ...
                { icon: <AiFillMoon />, name: 'Optional', filter:(p) => p === 'secondButton', action: () => alert('third') },
            ]} />
        </>
    )
}
